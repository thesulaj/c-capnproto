...
static int init_fp(struct capn *c, FILE *f, struct capn_stream *z, int packed) {
     ...
     * serialized with the standard framing format. From
     * https://capnproto.org/encoding.html:
     *
     * When transmitting over a stream, the following should be sent. All
     * integers are unsigned and little-endian. (4 bytes) The number of
     * segments, minus one (since there is always at least one segment). (N * 4
     * bytes) The size of each segment, in words. (0 or 4 bytes) Padding up to
     * the next word boundary. The content of each segment, in order.
     */
...
static int header_render(struct capn *c,
    ...
    header[headerlen - 1] =
        0; /* Zero out the spare position in the header sizes */
...
static int64_t capn_write_mem_packed(struct capn *c, uint8_t *p, size_t sz) {
    ...
    header = (uint32_t *) (p + headersz + 2); /* must reserve two bytes for
                                                 worst case expansion */

    if (sz < headersz * 2 + 2) /* We must have space for temporary writing of
                                  header to deflate */
        return -1;
    ...
    // pack the headers
    ret = capn_deflate(&z);
    if (ret != 0 || z.avail_in != 0) return -1;
...
int64_t capn_write_mem(struct capn *c, uint8_t *p, size_t sz, int packed) {
    ...
    if (c->segnum == 0) return -1;

    if (packed) return capn_write_mem_packed(c, p, sz);

    root = capn_root(c);
    header_calc(c, &headerlen, &headersz);
    header = (uint32_t *) p;

    if (sz < headersz) return -1;

    ret = header_render(c, root.seg, header, headerlen, &datasz);
    if (ret != 0) return -1;

    if (sz < headersz + datasz) return -1;
...
static int _write_fd(ssize_t (*write_fd)(int fd, const void *p, size_t count),
                     int fd,
                     void *p,
                     size_t count) {
    ssize_t ret;
...
int capn_write_fd(struct capn *c,
    ...
    if (sizeof(buf) < headersz) return -1;
